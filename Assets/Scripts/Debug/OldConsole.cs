...
using UnityEngine;
    ...
    private void OnGUI()
    {
        ...
        if(isMessage)
        {
            GUILayout.Window(1, new Rect(Screen.width / 2 - 100, Screen.height / 2 - 100, 200, 50), delegate (int i)
                   {
        ...
        GUILayout.Window(0, new Rect(10, 10, 200, 100), delegate (int id)
        {
            ...
            if (GUILayout.Button("Fly " + (cameraControll.PlayerControll?.isFly).ToString()))
            {
